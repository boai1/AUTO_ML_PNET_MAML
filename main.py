...
argparser.add_argument("--algorithm", choices=["maml", "pnet"], default="maml",
                       help="What meta-learning algorithm to use")
argparser.add_argument("--num_ways", default=5, type=int, help="Number of classes per task")
argparser.add_argument("--num_support_shots", default=1, type=int,
                       help="Number of examples/shots per class in support sets")
argparser.add_argument("--num_query_shots", default=15, type=int,
                       help="Number of examples/shots per class in query sets")
argparser.add_argument("--meta_batch_size", default=1, type=int, help="Number of tasks in a meta-batch")
argparser.add_argument("--val_interval", default=500, type=int,
                       help="After how many meta-updates we perform meta-validation")
argparser.add_argument("--num_eval_tasks", default=1000, type=int, help="Number of tasks used for validation/testing")
argparser.add_argument("--num_train_episodes", default=40000, type=int,
                       help="Number of meta-updates to make during training")
...
argparser.add_argument("--second_order", default=True, action="store_true",
                       help="Whether to use second-order gradients")
argparser.add_argument("--dataset", default="omniglot", type=str,
                       help="dataset to use")  # DO NOT CHANGE THIS FROM DEFAULT (omniglot)
argparser.add_argument("--T", default=3, type=int,
                       help="Number of inner gradient update steps (inner = on the support set)")
...
argparser.add_argument("--dev", default="cuda:0", help="GPU ID to use")
...
if args.algorithm == "maml":
    if args.second_order:
        RDIR = f"./results/empirical_q2_{args.algorithm}_second_order_seed_{args.seed}_T_{args.T}/"
    else:
        RDIR = f"./results/empirical_q2_{args.algorithm}_first_order_seed_{args.seed}/"
else:
    RDIR = f"./results/empirical_q1_{args.algorithm}_seed_{args.seed}/"
...
model = constr(num_ways=args.num_ways, input_size=args.img_size ** 2, T=args.T, second_order=args.second_order,
               rgb=args.rgb, img_size=args.img_size)  # images are of size 28x28
